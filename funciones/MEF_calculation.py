...
from funciones.compute_eigenvector import*
from funciones.determinant import*
from funciones.eigenvalues import*
from funciones.transpose import*
from funciones.print_paraview import*
from funciones.transpose import*
from funciones.determinant import*
from funciones.kloc_tet import*
...
def calculate_FEM (steps, disp_vert, label_test):
    ...
    v_nodes=[]
    fcell= open('data/nodes.txt','r') # x, y, z
    ...
    v_mesh=[]
    fcell= open('data/mesh.txt','r') # n1, n2, n3, n4
    for line in fcell.readlines() :
    ...
    v_mechanical=[] 
    fcell= open('data/mechanical_mat.txt','r') # n1
    iteraux=0
    for line in fcell.readlines() :
        vecaux=[float(x) for x in line.split()]
        iteraux+=1
    ...
    fload= open('data/load_reaction_force_'+label_test+'.txt','w')
    fdisp= open('data/vertical_disp_'+label_test+'.txt','w')
    ftime= open('data/timeinfo_'+label_test+'.txt','w')
...
#-------------------------------------------------------------------------#
        ...
        fload= open('data/load_reaction_force_'+label_test+'.txt','a')
        fdisp= open('data/vertical_disp_'+label_test+'.txt','a')
        ftime= open('data/timeinfo_'+label_test+'.txt','a')
    ...
    load_disp=[]
    fload= open('data/load_reaction_force_'+label_test+'.txt','r')
    for line in fload.readlines() :
        vecaux=[float(x) for x in line.split()]
        load_disp.append([vecaux[0]])
    fload.close()

    fdisp= open('data/vertical_disp_'+label_test+'.txt','r')
    ind=0
